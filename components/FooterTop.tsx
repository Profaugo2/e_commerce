...
const data: ContactItemData[] = [
    ...
    {
        title: "Email Us",
        subtitle: "nedyshop88@gmail.com",
        icon: (<Mail className='h-6 w-6 text-gray-600 group-hover:text-primary transition-colors'/>),
    },
]
const FooterTop = () => {
  return ( <div className='w-full border-b grid grid-cols-2 lg:grid-cols-4 gap-8'>
     {data?.map((item,index)=>(
        <div 
        key={index}
        className='flex items-center gap-3 group hover:bg-gray-50 p-4 transition-colors'>{item?.icon}
        <div>
